...
const Categories = () => {
  ...
  const [formData, setFormData] = useState<Partial<Category>>({
    ...
    singlePrompt: '',
    promptType: 'single',
    defaultNumToGenerate: 1, // Changed from 5 to 1 as default
    priority: 0,
    active: true,
    contentType: 'hack'
  });
  ...
  const handleSubmit = async () => {
    try {
      ...
      const finalFormData = {
        ...
        // For new categories, always generate exactly 1 content item
        defaultNumToGenerate: dialogMode === 'add' ? 1 : (formData.defaultNumToGenerate || 1),
          ...
          <Box sx={{ pt: 1 }}>
            ...
            <Box display="flex" gap={2} mt={2}>
              ...
              <TextField
                ...
                type="number"
                value={formData.defaultNumToGenerate || 1}
                onChange={handleInputChange}
                fullWidth
                margin="normal"
                InputProps={{ 
                  inputProps: { min: 1, max: 20 },
                  readOnly: dialogMode === 'add' // Make field read-only when adding a new category
                }}
                disabled={dialogMode === 'add'} // Disable the field when adding a new category
                helperText={dialogMode === 'add' 
                  ? "When creating a new category, exactly 1 content item will be generated" 
                  : "How many items to generate by default"}
              />
            </Box>
